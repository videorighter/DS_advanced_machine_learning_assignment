...
def cosine_kernel(x, train, h):
    ...
    k = 0
    for point in train:
        is_inside = (np.pi/4)*np.cos(np.pi/2*(x-point)/h)
        if np.abs(x-point) > h:
            is_inside = 0   
        k += is_inside
    return (k/len(train))/h
...
def gaussian_kernel(x, train, h):
    ...
    k = 0
    for point in train:
        k += 1/np.sqrt(2*np.pi)*np.power(np.e, -((x-point)/h)**2/2)
...
def gaussian_2d_kernel(x, train, h):
    ...
    for point in train:
        tmp.append((x-point)/h)
    k = np.sum(multivariate_normal.pdf(tmp, mu_vec, cov_mat))
    return (k/len(train))/(h**2)
